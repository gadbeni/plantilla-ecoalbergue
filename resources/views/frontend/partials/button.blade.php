<div class="social-media-nav">
    <input type="checkbox" id="social-media-toggle">
    <div class="social-media-links">
        <a href="https://www.facebook.com/Aguaysalecoalbergue" target="_black" class="bi bi-facebook" aria-label="Facebook"></a>
        <a href="https://wa.me/{{setting('contacto.whatsapp')}}" target="_black" class="bi bi-whatsapp" aria-label="WhatsApp"></a>
        <a href="https://m.me/AguaYSalEcoalbergue" target="_black" class="bi bi-messenger" aria-label="Messenger"></a>
    </div>
    <div class="social-media-toggle">
        <label for="social-media-toggle">
            <i class="bi bi-wechat"></i>
            <i class="bi bi-x"></i>
        </label>
    </div>
</div>
